    ...
    step.pages.forEach((page) =>
      page.sections.forEach((section) =>
        section.fields.forEach((field) => {
          ...
          switch (field.type) {
            ...
            case "dropdown":
              ...
              if (typeof min === "number") {
                schema = schema.min(min, `Select at least ${min} options`);
              }
              if (typeof max === "number") {
                schema = schema.max(max, `Select at most ${max} options`);
              }
              break;
            case "radio":
              schema = Yup.string();
          ...
          // entryCodes → oneOf
          if (Array.isArray(field.validation?.entryCodes) && field.validation.entryCodes.length > 0) {
