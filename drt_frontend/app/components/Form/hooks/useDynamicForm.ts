...
export function useDynamicForm(parsedSteps: Step[]) {
  ...
  const handleVerifyAndSubmit = async (format: Format) => {
    console.log('handleVerifyAndSubmit', format)
    const url = `/drt/api/submission/?format=${format}`    // note the slash before '?'
    console.log("🛰️  Fetching:", url)
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/drt/api/submission/?format=${format}`,
        {
          method: 'POST',
          headers: {
