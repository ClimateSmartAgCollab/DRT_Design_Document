...
const FillQuestionnairePage = (p0: boolean) => {
  ...
  useEffect(() => {
    const fetchQuestionnaire = async () => {
      try {
        ...
        const parsedQuestionnaire = data.questionnaire
          ? JSON.parse(data.questionnaire)
          : null;
        ...
        if (response.ok && parsedQuestionnaire) {
          ...
          setError(
            parsedQuestionnaire?.error || "Failed to load questionnaire."
          );
        }
      } catch (err) {
