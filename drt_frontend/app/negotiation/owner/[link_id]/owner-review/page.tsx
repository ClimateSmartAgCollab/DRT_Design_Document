...
// Re-use the exact same form-schema logic:
...
export default function OwnerReviewPage() {
  ...
  const parsedSteps = useMemo(
    () => sortStepsByReferences(parseJsonToFormStructure()),
    []
  );
  ...
  const actionMutation = useMutation<{ message: string }, Error, string>({
    ...
    onError(err) {
      ...
      setGlobalComments((c) => c);
      console.error("Action error:", err);
      setFieldComments((fc) => ({ ...fc }));
      setGlobalComments((gc) => gc);
